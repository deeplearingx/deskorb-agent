...
from quick_actions import QUICK_ACTIONS, build_quick_action_prompt, get_quick_action
from workflow_records import WorkflowRecordRepository
from workflow_sources import MAX_FILES, MAX_TOTAL_TEXT_CHARS, MaterialError, extract_file_materials
from workflows import WORKFLOWS, WorkflowSource, build_workflow_prompt, get_workflow, parse_workflow_result
...
def _state_text(state, key, default):
    """Return a usable persisted string, treating legacy null/'None' values as absent."""
...
class Overlay:
    def __init__(self):
        ...
        self.workflow_repository = WorkflowRecordRepository()
        self.workflow_mode = "chat"
        self.workflow_selected_id = WORKFLOWS[0].id
        self.workflow_sources: list[WorkflowSource] = []
        self.workflow_shots: list[dict] = []
        self._workflow_shots_by_source: dict[int, list[dict]] = {}
        self._workflow_run = None
        self._workflow_stream_buffer = ""
        self._workflow_tail_started = False
        self._word_read_request = None
        self._word_read_sequence = 0
        self._word_read_timeout_after = None
    ...
    def _build(self):
        ...
        self._build_workflow_center()
        self._build_records_center()
    ...
    def _check_for_update(self):
        ...
        # This Codex adaptation is not released from the upstream Claude repository,
        # so comparing its version against upstream tags would produce a false update.
        return
        def work():
            try:
    ...
    def _apply_share_visibility(self):
        ...
        grabs so Codex's screenshots still never contain the overlay. The affinity is bound
        ...
        try:
            ...
            # — never leak the overlay into the screenshots we send Codex.
    ...
    def _build_mode_nav(self):
        """Top-level navigation keeps chat, workflows, and saved records separate."""
        wrap = tk.Frame(self.root, bg=T["bg"])
        wrap.pack(fill="x", padx=self.px(12), pady=(self.px(5), self.px(3)))
        self.mode_nav = wrap
        self.mode_buttons = {}
        for mode, label in (("chat", "Chat"), ("workflows", "Workflows"), ("records", "Records")):
            button = tk.Button(
                wrap, text=label, command=lambda value=mode: self._show_mode(value),
                bg=T["field"], fg=T["text"], activebackground=T["accent"],
                activeforeground=T["on_accent"], font=self.f_small, relief="flat", bd=0,
                padx=self.px(9), pady=self.px(3), cursor="hand2", takefocus=0,
            )
            button.pack(side="left", padx=(0, self.px(4)))
            self.mode_buttons[mode] = button
        self._paint_mode_nav()

    def _paint_mode_nav(self):
        for mode, button in getattr(self, "mode_buttons", {}).items():
            active = mode == getattr(self, "workflow_mode", "chat")
            try:
                button.configure(bg=T["accent"] if active else T["field"],
                                 fg=T["on_accent"] if active else T["text"])
            except Exception:
                pass

    def _show_mode(self, mode):
        if mode not in {"chat", "workflows", "records"}:
            return
        if self.busy and mode != "chat":
            self.add_sys("Finish (or Stop) the current reply before changing workspace modes.")
            return
        self.workflow_mode = mode
        for frame in (getattr(self, "chat_wrap", None), getattr(self, "workflow_frame", None),
                      getattr(self, "records_frame", None), getattr(self, "input_wrap", None),
                      getattr(self, "quick_actions_frame", None), getattr(self, "chat_word_frame", None)):
            try:
                frame.pack_forget()
            except Exception:
                pass
        if mode == "chat":
            ...
            if hasattr(self, "quick_actions_frame"):
                self.quick_actions_frame.pack(fill="x", side="bottom", padx=self.px(12), pady=(0, self.px(3)))
            if hasattr(self, "chat_word_frame"):
                self.chat_word_frame.pack(fill="x", side="bottom", padx=self.px(12), pady=(0, self.px(3)))
            if hasattr(self, "chat_wrap"):
                self.chat_wrap.pack(fill="both", expand=True, side="top")
            if hasattr(self, "root") and hasattr(self, "entry"):
                self.root.after(10, self.entry.focus_set)
        elif mode == "workflows":
            self.workflow_frame.pack(fill="both", expand=True, side="top")
            self._refresh_workflow_form()
        else:
            self.records_frame.pack(fill="both", expand=True, side="top")
            self._refresh_records()
        self._paint_mode_nav()

    def _build_workflow_center(self):
        frame = tk.Frame(self.root, bg=T["bg"])
        self.workflow_frame = frame
        tk.Label(frame, text="Communication and meeting workflows", bg=T["bg"], fg=T["text"],
                 font=self.f_title, anchor="w").pack(fill="x", padx=self.px(16), pady=(self.px(10), self.px(4)))
        self.workflow_choice = tk.StringVar(value=self.workflow_selected_id)
        labels = {workflow.label: workflow.id for workflow in WORKFLOWS}
        choice = tk.OptionMenu(frame, self.workflow_choice, *labels.keys(), command=lambda label: self._select_workflow(labels[label]))
        choice.configure(bg=T["field"], fg=T["text"], activebackground=T["accent"],
                         activeforeground=T["on_accent"], relief="flat", font=self.f_small,
                         highlightthickness=0)
        choice["menu"].configure(bg=T["field"], fg=T["text"])
        choice.pack(fill="x", padx=self.px(16), pady=(0, self.px(6)))
        self.workflow_params_frame = tk.Frame(frame, bg=T["bg"])
        self.workflow_params_frame.pack(fill="x", padx=self.px(16))
        self.workflow_parameter_vars = {}

        source_bar = tk.Frame(frame, bg=T["bg"])
        source_bar.pack(fill="x", padx=self.px(16), pady=(self.px(8), self.px(3)))
        self.workflow_material_buttons = []
        source_rows = (tk.Frame(source_bar, bg=T["bg"]), tk.Frame(source_bar, bg=T["bg"]))
        source_rows[0].pack(fill="x")
        source_rows[1].pack(fill="x", pady=(self.px(3), 0))
        for index, (label, command) in enumerate((
            ("Add active window", self._add_workflow_window),
            ("Read current Word", self._add_workflow_word),
            ("Add files", self._add_workflow_files),
            ("Remove selected", self._remove_workflow_source),
        )):
            button = tk.Button(source_rows[index // 2], text=label, command=command, bg=T["field"], fg=T["text"],
                               relief="flat", font=self.f_small)
            button.pack(side="left", padx=(0, self.px(4)))
            self.workflow_material_buttons.append(button)
        self.workflow_source_list = tk.Listbox(frame, bg=T["field"], fg=T["text"], bd=0,
                                               selectbackground=T["accent"], height=4,
                                               font=self.f_small, activestyle="none")
        self.workflow_source_list.pack(fill="x", padx=self.px(16), pady=(0, self.px(5)))
        tk.Label(frame, text="Pasted notes (optional)", bg=T["bg"], fg=T["muted"],
                 font=self.f_small, anchor="w").pack(fill="x", padx=self.px(16))
        self.workflow_text = tk.Text(frame, bg=T["field"], fg=T["text"], bd=0, height=5,
                                     wrap="word", font=self.f_small, insertbackground=T["accent"])
        self.workflow_text.pack(fill="both", expand=True, padx=self.px(16), pady=(self.px(2), self.px(7)))
        self.workflow_text.bind("<KeyRelease>", lambda _event: self._refresh_workflow_sources())
        self.workflow_run_button = tk.Button(frame, text="Run workflow", command=self._run_selected_workflow,
                                             bg=T["accent"], fg=T["on_accent"], relief="flat",
                                             font=self.f_chip, pady=self.px(5))
        self.workflow_run_button.pack(fill="x", padx=self.px(16), pady=(0, self.px(12)))
        self._refresh_workflow_form()
        self._refresh_workflow_controls()

    def _select_workflow(self, workflow_id):
        self.workflow_selected_id = workflow_id
        self._refresh_workflow_form()

    def _refresh_workflow_form(self):
        if not hasattr(self, "workflow_params_frame"):
            return
        for child in self.workflow_params_frame.winfo_children():
            child.destroy()
        self.workflow_parameter_vars = {}
        workflow = get_workflow(self.workflow_selected_id)
        for parameter in workflow.parameters:
            row = tk.Frame(self.workflow_params_frame, bg=T["bg"])
            row.pack(fill="x", pady=(0, self.px(3)))
            tk.Label(row, text=parameter.label, bg=T["bg"], fg=T["muted"], font=self.f_small,
                     width=13, anchor="w").pack(side="left")
            variable = tk.StringVar(value=parameter.default)
            self.workflow_parameter_vars[parameter.id] = variable
            tk.Entry(row, textvariable=variable, bg=T["field"], fg=T["text"], bd=0,
                     insertbackground=T["accent"], font=self.f_small).pack(side="left", fill="x", expand=True)
        self._refresh_workflow_sources()

    def _workflow_parameters(self):
        return {name: variable.get().strip() for name, variable in self.workflow_parameter_vars.items()}

    def _workflow_text_source(self):
        if not hasattr(self, "workflow_text"):
            return None
        text = self.workflow_text.get("1.0", "end").strip()
        return WorkflowSource("text", "Pasted notes", text) if text else None

    def _refresh_workflow_sources(self):
        box = getattr(self, "workflow_source_list", None)
        if box is None:
            return
        try:
            box.delete(0, "end")
            for source in self.workflow_sources:
                status = f" · {source.status}" if source.status else ""
                box.insert("end", f"{source.source_type}: {source.summary}{status}")
            if self._workflow_text_source() is not None:
                box.insert("end", "text: Pasted notes")
        except Exception:
            pass

    def _workflow_input_busy(self):
        return self.busy or getattr(self, "_word_read_request", None) is not None

    def _refresh_workflow_controls(self):
        state = "disabled" if self._workflow_input_busy() else "normal"
        for button in getattr(self, "workflow_material_buttons", []):
            try:
                button.configure(state=state)
            except Exception:
                pass
        try:
            self.workflow_text.configure(state=state)
        except Exception:
            pass
        try:
            self.workflow_run_button.configure(state=state)
        except Exception:
            pass

    def _add_workflow_window(self):
        if self._workflow_input_busy():
            self.add_sys("Finish (or Stop) the current reply before adding a window.")
            return
        shots, err = self._grab_window_shot()
        if not shots:
            detail = f" ({type(err).__name__}: {err})" if err else ""
            self.add_err("Couldn't capture the active window. Focus the target app and try again." + detail)
            return
        title = shots[0].get("window") or "untitled window"
        source = WorkflowSource("active_window", title, "")
        self.workflow_sources.append(source)
        self._workflow_shot_map()[id(source)] = list(shots)
        self._sync_workflow_shots()
        self._refresh_workflow_sources()

    def _add_workflow_word(self):
        if self._workflow_input_busy():
            self.add_sys("Finish the current reply or Word read before adding another material.")
            return
        hwnd = self._capture_target_hwnd()
        if not hwnd or not is_word_window(hwnd):
            self.add_err("Focus a Microsoft Word document before opening the Overlay, then try again.")
            return
        self._word_read_sequence = getattr(self, "_word_read_sequence", 0) + 1
        request_id = self._word_read_sequence
        self._word_read_request = request_id
        self._refresh_workflow_controls()
        self.add_sys("Reading the current Word document…")
        try:
            self._word_read_timeout_after = self.root.after(
                15_000, lambda: self._expire_workflow_word_read(request_id)
            )
        except Exception:
            self._word_read_timeout_after = None
        threading.Thread(
            target=self._read_workflow_word_bg, args=(request_id, hwnd),
            name="workflow-word-read", daemon=True,
        ).start()

    def _read_workflow_word_bg(self, request_id, hwnd):
        try:
            self.ui_q.put(("workflow_word_read", (request_id, read_active_word_document(hwnd), None)))
        except Exception as exc:
            self.ui_q.put(("workflow_word_read", (request_id, None, exc)))

    def _expire_workflow_word_read(self, request_id):
        if getattr(self, "_word_read_request", None) != request_id:
            return
        self._word_read_request = None
        self._word_read_timeout_after = None
        self._refresh_workflow_controls()
    ...
    def _finish_workflow_word_read(self, request_id, material: WordMaterial | None = None, error=None):
        """Accept only the current asynchronous Word-read result, never a late one."""
        if getattr(self, "_word_read_request", None) != request_id:
            return
        self._word_read_request = None
        timeout_after = getattr(self, "_word_read_timeout_after", None)
        self._word_read_timeout_after = None
        ...
        retained = [
            source for source in self.workflow_sources
            if not (source.source_type == "word_document" and source.source_id == material.document_id)
        ]
        extracted_chars = sum(
            len(source.content) for source in retained if source.source_type in {"file", "word_document"}
        )
        if extracted_chars + material.character_count > MAX_TOTAL_TEXT_CHARS:
            self.add_err(f"Selected materials exceed the {MAX_TOTAL_TEXT_CHARS:,}-character text limit.")
            return
        state = "unsaved changes" if material.has_unsaved_changes else "saved"
        source = WorkflowSource(
            "word_document", material.name, material.text, material.document_id,
            f"{material.character_count:,} chars · {state}",
        )
        insert_at = next(
            (index for index, previous in enumerate(self.workflow_sources)
             if previous.source_type == "word_document" and previous.source_id == material.document_id),
            None,
        )
        if insert_at is None:
            self.workflow_sources.append(source)
        else:
            self.workflow_sources[insert_at] = source
        self._refresh_workflow_sources()
        self.add_sys(f"Read Word document: {material.name} ({material.character_count:,} chars, {state}).")

    def _add_workflow_files(self):
        if self._workflow_input_busy():
            self.add_sys("Finish (or Stop) the current reply before adding files.")
            return
        selected = filedialog.askopenfilenames(
            parent=self.root, title="Add meeting materials",
            filetypes=[("Supported materials", "*.txt *.md *.markdown *.docx *.pdf"), ("All files", "*.*")],
        )
        if not selected:
            return
        try:
            materials = extract_file_materials(selected)
        except MaterialError as exc:
            self.add_err(str(exc))
            return
        existing_files = sum(source.source_type == "file" for source in self.workflow_sources)
        if existing_files + len(materials) > MAX_FILES:
            self.add_err(f"Select at most {MAX_FILES} files for one workflow.")
            return
        existing_chars = sum(
            len(source.content) for source in self.workflow_sources
            if source.source_type in {"file", "word_document"}
        )
        if existing_chars + sum(len(material.text) for material in materials) > MAX_TOTAL_TEXT_CHARS:
            self.add_err(f"Selected materials exceed the {MAX_TOTAL_TEXT_CHARS:,}-character text limit.")
            return
        self.workflow_sources.extend(WorkflowSource("file", material.name, material.text) for material in materials)
        self._refresh_workflow_sources()

    def _remove_workflow_source(self):
        try:
            selected = self.workflow_source_list.curselection()
            index = int(selected[0]) if selected else -1
        except Exception:
            index = -1
        if 0 <= index < len(self.workflow_sources):
            source = self.workflow_sources.pop(index)
            if source.source_type == "active_window":
                self._workflow_shot_map().pop(id(source), None)
                self._sync_workflow_shots()
            self._refresh_workflow_sources()
        elif index == len(self.workflow_sources) and hasattr(self, "workflow_text"):
            self.workflow_text.delete("1.0", "end")
            self._refresh_workflow_sources()

    def _run_selected_workflow(self):
        if self._workflow_input_busy():
            self.add_sys("Finish (or Stop) the current reply before running a workflow.")
            return
        sources = list(self.workflow_sources)
        pasted = self._workflow_text_source()
        if pasted:
            sources.append(pasted)
        if not sources:
            self.add_err("Add an active window, pasted notes, or supported files before running a workflow.")
            return
        try:
            workflow = get_workflow(self.workflow_selected_id)
            prompt = build_workflow_prompt(workflow.id, sources, self._workflow_parameters())
        ...
        for source in sources:
            if source.source_type == "active_window":
                summaries.append({"kind": "active_window", "window_title": source.summary})
            elif source.source_type == "file":
                summaries.append({"kind": "file", "name": source.summary})
            elif source.source_type == "word_document":
                summaries.append({"kind": "word_document", "name": source.summary,
                                  "character_count": str(len(source.content))})
            else:
                summaries.append({"kind": "text", "title": source.summary})
        self._workflow_run = {"workflow_id": workflow.id, "source_summaries": summaries}
        self._workflow_stream_buffer = ""
        self._workflow_tail_started = False
        self.add_user(f"Workflow: {workflow.label} ({len(sources)} material{'s' if len(sources) != 1 else ''})")
        self._show_mode("chat")
        self._dispatch_turn(prompt, list(self.workflow_shots))

    def _workflow_visible_delta(self, text):
        """Hold the structured comment tail back while ordinary Markdown keeps streaming."""
        self._workflow_stream_buffer += "" if text is None else str(text)
        marker = "<!-- CODEX_OVERLAY_RECORD"
        if self._workflow_tail_started:
            return ""
        found = self._workflow_stream_buffer.find(marker)
        if found >= 0:
            self._workflow_tail_started = True
            visible = self._workflow_stream_buffer[:found]
            self._workflow_stream_buffer = self._workflow_stream_buffer[found:]
            return visible
        max_prefix = min(len(marker) - 1, len(self._workflow_stream_buffer))
        keep = 0
        for size in range(max_prefix, 0, -1):
            if self._workflow_stream_buffer.endswith(marker[:size]):
                keep = size
                break
        visible = self._workflow_stream_buffer[:-keep] if keep else self._workflow_stream_buffer
        self._workflow_stream_buffer = self._workflow_stream_buffer[-keep:] if keep else ""
        return visible

    def _complete_workflow_run(self):
        run = self._workflow_run
        if not run:
            return
        parsed = parse_workflow_result(self._turn_raw, run["workflow_id"])
        if not self._workflow_tail_started and self._workflow_stream_buffer:
            self._md_feed(self._workflow_stream_buffer)
        self._workflow_stream_buffer = ""
        self._workflow_tail_started = False
        self._turn_raw = parsed.markdown
        try:
            self.workflow_repository.create_record(
                workflow_id=run["workflow_id"], markdown=parsed.markdown,
                structured_data=parsed.data, source_summaries=run["source_summaries"],
            )
            self.add_sys("Workflow result saved to Records.")
        except Exception as exc:
            self.add_err(f"Couldn't save workflow record: {type(exc).__name__}: {exc}")
        finally:
            self._workflow_run = None
            self._clear_workflow_materials()

    def _cancel_workflow_run(self):
        had_run = self._workflow_run is not None
        self._workflow_run = None
        self._workflow_stream_buffer = ""
        self._workflow_tail_started = False
        if had_run:
    ...
    def _clear_workflow_materials(self):
        """Drop transient source bodies once a workflow no longer needs them."""
        self.workflow_sources = []
        self.workflow_shots = []
        self._workflow_shot_map().clear()
        try:
            self.workflow_text.delete("1.0", "end")
        except Exception:
            pass
    ...
    def _workflow_shot_map(self):
        """Return the transient active-window screenshot groups keyed by source identity."""
        if not hasattr(self, "_workflow_shots_by_source"):
            self._workflow_shots_by_source = {}
        return self._workflow_shots_by_source

    def _sync_workflow_shots(self):
        """Keep dispatch screenshots aligned with the remaining active-window sources."""
        shot_map = self._workflow_shot_map()
        self.workflow_shots = [
            shot
            for source in self.workflow_sources
            for shot in shot_map.get(id(source), [])
        ]

    def _build_records_center(self):
        frame = tk.Frame(self.root, bg=T["bg"])
        self.records_frame = frame
        top = tk.Frame(frame, bg=T["bg"])
        top.pack(fill="x", padx=self.px(16), pady=(self.px(10), self.px(5)))
        tk.Label(top, text="Saved records", bg=T["bg"], fg=T["text"], font=self.f_title).pack(side="left")
        self.record_filter = tk.StringVar(value="all")
        for label, value in (("All", "all"), ("Open", "open"), ("Near due", "near_due"), ("Overdue", "overdue")):
            tk.Radiobutton(top, text=label, value=value, variable=self.record_filter, command=self._refresh_records,
                           bg=T["bg"], fg=T["text"], selectcolor=T["field"], activebackground=T["bg"],
                           font=self.f_small).pack(side="left", padx=(self.px(6), 0))
        self.records_list = tk.Listbox(frame, bg=T["field"], fg=T["text"], bd=0, height=8,
                                       selectbackground=T["accent"], font=self.f_small, activestyle="none")
        self.records_list.pack(fill="x", padx=self.px(16), pady=(0, self.px(5)))
        self.records_list.bind("<<ListboxSelect>>", lambda _event: self._show_selected_record())
        self.record_detail = tk.Text(frame, bg=T["field"], fg=T["text"], bd=0, wrap="word", font=self.f_small,
                                     height=12, state="disabled")
        self.record_detail.pack(fill="both", expand=True, padx=self.px(16), pady=(0, self.px(5)))
        actions = tk.Frame(frame, bg=T["bg"])
        actions.pack(fill="x", padx=self.px(16), pady=(0, self.px(12)))
        tk.Button(actions, text="Edit record", command=self._edit_selected_record, bg=T["field"], fg=T["text"],
                  relief="flat", font=self.f_small).pack(side="left", padx=(0, self.px(4)))
        tk.Button(actions, text="Edit task", command=self._edit_selected_task, bg=T["field"], fg=T["text"],
                  relief="flat", font=self.f_small).pack(side="left", padx=(0, self.px(4)))
        tk.Button(actions, text="Copy", command=self._copy_selected_record, bg=T["field"], fg=T["text"],
                  relief="flat", font=self.f_small).pack(side="left", padx=(0, self.px(4)))
        tk.Button(actions, text="Export Markdown", command=self._export_selected_record, bg=T["field"], fg=T["text"],
                  relief="flat", font=self.f_small).pack(side="left")
        self._record_rows = []
    ...
    def _refresh_records(self):
        if not hasattr(self, "records_list"):
            return
        try:
            self._record_rows = self.workflow_repository.list_records(self.record_filter.get())
        except Exception as exc:
            self.add_err(f"Couldn't load records: {type(exc).__name__}: {exc}")
            return
        self.records_list.delete(0, "end")
        for record in self._record_rows:
            self.records_list.insert("end", f"{record.created_at.astimezone().strftime('%Y-%m-%d %H:%M')}  {record.workflow_id}")
    ...
    def _selected_record(self):
        selection = self.records_list.curselection() if hasattr(self, "records_list") else ()
        return self._record_rows[int(selection[0])] if selection else None

    def _show_selected_record(self):
        record = self._selected_record()
        if not record:
            return
        detail = self._record_markdown(record)
        self.record_detail.configure(state="normal")
        self.record_detail.delete("1.0", "end")
        self.record_detail.insert("1.0", detail)
        self.record_detail.configure(state="disabled")

    @staticmethod
    def _record_markdown(record):
        task_text = "\n".join(
            f"- [{'x' if item.status == 'completed' else ' '}] {item.task} | {item.owner or 'Unassigned'} | {item.due_date or 'No due date'}"
            for item in record.action_items
        )
        return record.markdown + (f"\n\n## Action items\n{task_text}" if task_text else "")

    def _refresh_and_reselect_record(self, record_id):
        self._refresh_records()
        for index, record in enumerate(self._record_rows):
            if record.id == record_id:
                self.records_list.selection_set(index)
                self.records_list.activate(index)
    ...
    def _edit_selected_record(self):
        ...
        window.title("Edit workflow record")
        window.configure(bg=T["bg"])
        window.geometry(f"{self.px(520)}x{self.px(420)}")
        editor = tk.Text(window, bg=T["field"], fg=T["text"], bd=0, wrap="word", font=self.f_small,
                         insertbackground=T["accent"])
        editor.pack(fill="both", expand=True, padx=self.px(12), pady=self.px(12))
        editor.insert("1.0", record.markdown)
        def save():
            try:
                self.workflow_repository.update_record(record.id, markdown=editor.get("1.0", "end").strip())
            except Exception as exc:
                self.add_err(f"Couldn't update record: {exc}")
                return
            window.destroy()
            self._refresh_and_reselect_record(record.id)
        tk.Button(window, text="Save", command=save, bg=T["accent"], fg=T["on_accent"], relief="flat",
                  font=self.f_small).pack(pady=(0, self.px(12)))

    def _edit_selected_task(self):
        record = self._selected_record()
        if not record or not record.action_items:
            self.add_sys("The selected record has no action items to edit.")
            return
        if len(record.action_items) == 1:
            task = record.action_items[0]
        else:
            choices = "\n".join(f"{index + 1}. {item.task}" for index, item in enumerate(record.action_items))
            selected = simpledialog.askinteger("Edit action item", f"Choose an action item:\n{choices}",
                                               parent=self.root, minvalue=1, maxvalue=len(record.action_items))
            if selected is None:
                return
            task = record.action_items[selected - 1]
        window = tk.Toplevel(self.root)
        window.title("Edit action item")
        window.configure(bg=T["bg"])
        values = {"task": tk.StringVar(value=task.task), "owner": tk.StringVar(value=task.owner),
                  "due_date": tk.StringVar(value=task.due_date), "priority": tk.StringVar(value=task.priority),
                  "status": tk.StringVar(value=task.status)}
        for name, variable in values.items():
            row = tk.Frame(window, bg=T["bg"])
            row.pack(fill="x", padx=self.px(12), pady=(self.px(5), 0))
            tk.Label(row, text=name.replace("_", " ").title(), bg=T["bg"], fg=T["text"], width=12,
                     anchor="w", font=self.f_small).pack(side="left")
            tk.Entry(row, textvariable=variable, bg=T["field"], fg=T["text"], bd=0, font=self.f_small,
                     insertbackground=T["accent"]).pack(side="left", fill="x", expand=True)
        def save():
            try:
                self.workflow_repository.update_action_item(task.id, **{name: variable.get() for name, variable in values.items()})
            except ValueError as exc:
                self.add_err(str(exc))
                return
            window.destroy()
            self._refresh_and_reselect_record(record.id)
        tk.Button(window, text="Save", command=save, bg=T["accent"], fg=T["on_accent"], relief="flat",
                  font=self.f_small).pack(pady=self.px(12))
    ...
    def _copy_selected_record(self):
        ...
        self.root.clipboard_append(self._record_markdown(record))
        self.add_sys("Record copied.")

    def _export_selected_record(self):
        record = self._selected_record()
        if not record:
            return
        target = filedialog.asksaveasfilename(parent=self.root, title="Export workflow record", defaultextension=".md",
                                               filetypes=[("Markdown", "*.md")])
        if not target:
            return
        try:
            Path(target).write_text(self._record_markdown(record), encoding="utf-8")
            self.add_sys("Record exported as Markdown.")
        except OSError as exc:
            self.add_err(f"Couldn't export record: {exc}")
    ...
    def _build_quick_actions(self):
        """Render fixed, screen-aware productivity actions above the text input."""
        wrap = tk.Frame(self.root, bg=T["bg"])
        wrap.pack(fill="x", side="bottom", padx=self.px(12), pady=(0, self.px(3)))
        self.quick_actions_frame = wrap
        self.quick_action_buttons = {}
        for action in QUICK_ACTIONS:
            button = tk.Button(
                wrap, text=action.label, command=lambda aid=action.id: self._run_quick_action(aid),
                bg=T["field"], fg=T["text"], activebackground=T["accent"],
            ...
            button.pack(side="left", padx=(0, self.px(4)))
            self.quick_action_buttons[action.id] = button
        self._refresh_quick_actions()

    def _build_chat_word_attachment(self):
        """Render the explicit, one-turn Word attachment control for Chat mode."""
        wrap = tk.Frame(self.root, bg=T["bg"])
        wrap.pack(fill="x", side="bottom", padx=self.px(12), pady=(0, self.px(3)))
    ...
    def _gear_items(self):
        ...
        return [
            ("      Connection settings…", self._open_connection_settings),
    ...
    def _refresh_quick_actions(self):
        ...
        for button in getattr(self, "quick_action_buttons", {}).values():
            try:
                button.configure(state=state)
            except Exception:
                pass
    ...
    def _refresh_chat_word_attachment(self):
        ...
        state = "disabled" if self.busy or reading else "normal"
        try:
            self.chat_word_button.configure(state=state)
    ...
    def _prune_chat(self):
        ...
        try:
            ...
            # one, deltas would append with no "Codex" header → a detached turn. Re-arm so the
    ...
    def _clip_bubble(text):
        ...
        Codex). Tk's canvas word-wrap is ~O(n²) on whitespace-free strings, so a pasted
    ...
    def add_think(self, text):
        # Stream extended-thinking tokens as a muted block under the Codex header, before
    ...
    def add_delta(self, text):
        if text is not None:
            raw = str(text)
            self._turn_raw += raw        # retain the full stream for workflow-tail parsing
            if self._workflow_run is not None:
                text = self._workflow_visible_delta(raw)
                if not text:
                    return
        self._ensure_header()
    ...
    # Codex streams Markdown token-by-token, so markup spans deltas. We commit BLOCK
    ...
    def _tool_chip(self, name, arg):
        """A compact rounded Codex-style tool pill embedded in the chat. render() rebuilds it from
    ...
    # ── per-message "Copy" button (ChatGPT/Codex-style) ──────────────────────────────
    ...
    def _show_cli_update_notice(self, info):
        ...
        self.add_sys(f"🔔 Your Codex CLI is out of date (v{inst} → v{latest}). The overlay is "
    ...
    def _show_cli_update_result(self, payload):
        ...
        if c is not None:
            try:
                ...
                pass
        if ok:
            self.add_sys(f"✅ Codex CLI updated to v{msg}. Click the button above to restart the "
    ...
    def _send_or_stop(self):
        if self.busy:
            self._cancel_workflow_run()
            self.worker.interrupt()
            self._set_status("stopping…")
            return
        if getattr(self, "_chat_word_read_request", None) is not None:
            self.add_sys("Finish reading the current Word document before sending a message.")
            return
        ...
        self.add_user(label)
        self._dispatch_turn(text, shots, images)

    def _dispatch_turn(self, text, shots=None, images=None, ephemeral=False):
        """Send a prepared prompt through the existing image/backend dispatch path."""
        images = list(images or [])
        if IMAGE_INPUT == "inline":
            paths = [s["path"] for s in (shots or [])] + list(images)
            prompt = self._inline_text(text, shots, images)
        else:
            prompt, paths = self._build_prompt(text, shots, images), []
        if ephemeral:
    ...
    def _send_chat_word_attachment(self, question):
        """Submit the visible Word attachment once, without leaking it into the chat display."""
        ...
        self.add_user(
            f"{question} [Word attachment: {material.name} · {material.character_count:,} chars]"
        )
    ...
    def _finish_chat_word_read(self, request_id, material: WordMaterial | None = None, error=None):
        """Accept only the current chat Word-read result, discarding late background results."""
        if getattr(self, "_chat_word_read_request", None) != request_id:
            return
        self._chat_word_read_request = None
        timeout_after = getattr(self, "_chat_word_read_timeout_after", None)
        ...
        if material is None:
            ...
            self._clear_chat_word_attachment()
            return
        if material.character_count > MAX_TOTAL_TEXT_CHARS:
            self.add_err(f"Word document exceeds the {MAX_TOTAL_TEXT_CHARS:,}-character text limit.")
            self._clear_chat_word_attachment()
            return
        self.chat_word_attachment = material
        self._refresh_chat_word_attachment()
        state = "unsaved changes" if material.has_unsaved_changes else "saved"
        self.add_user(f"Word attached: {material.name} ({material.character_count:,} chars, {state}).")

    def _run_quick_action(self, action_id):
        """Capture only the last active external window and submit a fixed office action."""
        if self.busy:
            self.add_sys("Finish (or Stop) the current reply before running a quick action.")
            return
        if getattr(self, "_chat_word_read_request", None) is not None:
            self.add_sys("Finish reading the current Word document before running a quick action.")
            return
        try:
            action = get_quick_action(action_id)
        except ValueError:
            self.add_err("That quick action is unavailable.")
            return
        ...
        title = shots[0].get("window") or "untitled window"
        self.add_user(f"Quick action: {action.label} - {title} [screenshot]")
        self._dispatch_turn(build_quick_action_prompt(action_id, title), shots)
    ...
    def toggle_collapse(self):
        if self.expanded:
            ...
            for w in (self.titlebar, self.hairline, self.chat_wrap, self.quick_actions_frame,
                      self.input_wrap, self.status_frame, self.statusline_frame):
                w.pack_forget()
            self._hide_edges()
            s = self.orb_size
            name = (self.overlay_name or "").strip()
            ...
            self.input_wrap.pack(fill="x", side="bottom")
            self.quick_actions_frame.pack(fill="x", side="bottom", padx=self.px(12),
                                          pady=(0, self.px(3)))
            self.chat_wrap.pack(fill="both", expand=True, side="top")
            self._show_edges()
    ...
    def toggle_visible(self):
        ...
        # "bring Codex to me", so raise+focus instead of making it vanish (the old
    ...
    def _model_menu(self, e):
        m = tk.Menu(self.root, tearoff=0, bg=T["field"], fg=T["text"],
                    activebackground=T["accent"], activeforeground=T["on_accent"], bd=0)
        m.add_command(label="Connection settings…", command=self._open_connection_settings)
        m.add_separator()
        for lbl, val in (("Auto (API when configured)", "auto"),
                         ("Codex persistent connection", "codex"),
                         ("OpenAI / compatible API", "api")):
            prefix = "✓  " if self._backend == val else "      "
            m.add_command(label=prefix + lbl, command=lambda v=val: self._switch_backend(v))
        m.add_separator()
        for lbl, val in MODELS:
            m.add_command(label=lbl, command=lambda v=val: self._switch_model(v))
        m.add_command(label="Custom model…", command=self._open_connection_settings)
        try:
            m.tk_popup(e.x_root, e.y_root)
        finally:
            m.grab_release()

    def _switch_backend(self, value):
        if self.busy:
    ...
    def _open_connection_settings(self):
        ...
        def save():
            ...
            if not (base.startswith("https://") or base.startswith("http://localhost")
                ...
                error_var.set("Use HTTPS, or HTTP only for localhost/127.0.0.1.")
                return
        ...
        tk.Button(buttons, text="Clear key", command=clear_key, bg=T["field"], fg=T["muted"],
                  activebackground=T["field"], relief="flat", padx=10, pady=5).pack(side="left", padx=5)
        tk.Button(buttons, text="Cancel", command=win.destroy, bg=T["field"], fg=T["muted"],
                  activebackground=T["field"], relief="flat", padx=10, pady=5).pack(side="left", padx=5)
        tk.Button(buttons, text="Save", command=save, bg=T["accent"], fg=T["on_accent"],
                  activebackground=T["accent_hi"], relief="flat", padx=14, pady=5).pack(side="left", padx=5)
    ...
    def _handle(self, kind, payload):
        if kind == "ready":
            ...
            self._active_backend = str(payload)
            self._refresh_statusline()
        elif kind == "connection_done":
            if isinstance(payload, dict):
                self._backend = str(payload.get("backend") or self._backend)
                self._active_backend = (f"auto→{payload.get('active')}"
        ...
        elif kind == "workflow_word_read":
            request_id, material, error = payload
            self._finish_workflow_word_read(request_id, material=material, error=error)
        elif kind == "chat_word_read":
            request_id, material, error = payload
            self._finish_chat_word_read(request_id, material=material, error=error)
        elif kind == "status":
    ...
    def quit(self):
        ...
        try:
            ...
            pass
        try:
            self.workflow_repository.close()
        except Exception:
            pass
        try:
        ...
        # Let the worker disconnect the agent before we tear down. If Codex is
