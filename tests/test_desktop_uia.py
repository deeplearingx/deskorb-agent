...
class DesktopUIATests(unittest.TestCase):
    def _observer(self):
        ...
        return DesktopUIA(desktop_factory=FakeDesktop, foreground_getter=lambda: 42), wrapper
    ...
    def test_invoke_verifies_a_toggle_state_change(self):
        ...
        observer = DesktopUIA(desktop_factory=FakeDesktop, foreground_getter=lambda: 42)
    ...
    def test_observe_rejects_a_stale_foreground_window(self):
        observer, _wrapper = self._observer()
        observer._foreground_getter = lambda: 77
        result = observer.observe_active_window(42)
        self.assertFalse(result["ok"])
        self.assertIn("active window changed", result["error"].lower())

    def test_set_value_rejects_oversized_or_nul_input(self):
        observer, wrapper = self._observer()
        control = observer.observe_active_window(42)["controls"][0]
        for value in ("x" * 4001, "safe\x00value"):
            with self.subTest(value_length=len(value)):
                result = observer.set_value(control["control_id"], 42, value)
                self.assertFalse(result["ok"])
                self.assertIsNone(wrapper.value)
