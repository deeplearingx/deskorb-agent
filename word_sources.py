...
def read_active_word_document(expected_hwnd: int) -> WordMaterial:
    """Read the main document story only when COM still points at the expected Word window."""
    if not is_word_window(expected_hwnd):
        raise WordMaterialError("Focus a Microsoft Word document, then open the Overlay and try again.")
...
def _read_document_from_application(application: Any, expected_hwnd: int) -> WordMaterial:
    try:
        active_hwnd = _normalise_hwnd(application.ActiveWindow.Hwnd)
    except Exception as exc:
        raise WordMaterialError("Word has no readable active document window.") from exc

    expected_root = _normalise_hwnd(root_window(expected_hwnd))
    active_root = _normalise_hwnd(root_window(active_hwnd))
    if not active_root or active_root != expected_root:
        raise WordMaterialError(
            "The active Word document does not match the Word window you opened the Overlay from. "
    ...
    try:
        ...
        text = _normalise_word_text(str(document.Content.Text or ""))
...
def _normalise_hwnd(hwnd: Any) -> int:
    try:
