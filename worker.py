...
class CodexWorker(threading.Thread):
    ...
    def ask_ephemeral(self, text: str, image_paths=None):
        """Run one turn without adding its input to the normal conversation memory."""
    ...
    def _launch_prefix(self) -> list[str]:
        ...
        if os.name == "nt" and self._codex.lower().endswith((".cmd", ".bat")):
            # `cmd /c` otherwise splits an npm-installed path such as
            # `C:\Users\Hu yubin\...\codex.cmd` at its first space. `call` makes
            # cmd treat the following quoted batch-file path as one command.
            return [os.environ.get("COMSPEC", "cmd.exe"), "/d", "/s", "/c", "call", self._codex]
        return [self._codex]
