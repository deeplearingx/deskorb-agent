"""Provider-neutral model protocol adapter for DeskOrb Agent.

DeskOrb keeps one internal, Responses-shaped transcript so the planner and its
tool loop stay vendor independent.  This module maps it to either OpenAI's
``/responses`` API or the widely supported ``/chat/completions`` API, then
normalizes chat-completion tool calls back to the internal representation.
"""
...
def provider_profile(provider: str | None, base_url: str | None) -> ProviderProfile:
    """Resolve protocol and a safe default endpoint without contacting a vendor."""
    ...
    if requested == "auto":
        if "deepseek.com" in lowered:
            ...
            # Preserve the existing DeskOrb default for private Responses
            # gateways.  OpenAI-compatible chat gateways can opt in explicitly.
            requested = "responses"
    defaults = {
    ...
    # config.py historically supplies OpenAI's base as a generic default.  An
    # explicit vendor choice must still work when the user only supplies that
    # vendor's API key, so replace that inherited default with the official URL.
...
class ModelAdapter:
    """Translate DeskOrb's canonical requests and replies for one provider."""
    ...
    def normalize_response(self, response: dict[str, Any]) -> dict[str, Any]:
        """Return the internal Responses-shaped result expected by AgentRuntime."""
        if self.protocol == "responses":
            return response
        ...
        for index, call in enumerate(message.get("tool_calls") or []):
            ...
            output.append({"type": "function_call", "call_id": str(call.get("id") or f"call_{index}"),
                           "name": name, "arguments": arguments if isinstance(arguments, str) else "{}"})
